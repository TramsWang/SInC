...
public class EstRelationMiner extends RelationMinerBasic {
    ...
    protected Rule getStartRule() {
        return new EstRule(targetRelation, kb.getRelation(targetRelation).totalCols(), new HashSet<>(), tabuSet, kb);
    }
        ...
        while (true) {
            ...
            try {
                ...
                for (int i = 0; i < beamwidth && null != beams[i]; i++) {
                    ...
                    List<SpecOprWithScore> estimated_specs = ((EstRule) r).estimateSpecializations();
