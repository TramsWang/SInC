...
//     void checkEntries(CachedRule::entriesType& expectedEntries, CachedRule::entriesType& actualEntries) {
//         ASSERT_EQ(expectedEntries.size(), actualEntries.size());
//         for (CachedRule::entryType* const& expected_entry: expectedEntries) {
//             for (CompliedBlock* const& cb : *expected_entry) {
//                 cb->buildIndices();
//             }
//         }
//         for (CachedRule::entryType* const& actual_entry: actualEntries) {
//             ASSERT_EQ(expectedEntries[0]->size(), actual_entry->size());
...
//             return false;
//         }
//         const int* const* expected_rows = expectedCb.getIndices().getAllRows();
//         const int* const* actual_rows = actualCb.getIndices().getAllRows();
//         int cols = expectedCb.getTotalCols();
...
//     void clearEntries(CachedRule::entriesType& entries) {
//         for (CachedRule::entryType* entry: entries) {
//             delete entry;
//         }
//         entries.clear();
//     }
